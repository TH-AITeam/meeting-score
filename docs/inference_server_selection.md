...
判断モデルが Issue #17 で決まったあと、それを動かす推論サーバを比較するためのドキュメント。

## 比較軸

| 軸 | 説明 |
| --- | --- |
| **採用モデルへの対応状況** | Issue #17 で選定したモデル（Qwen2.5 / Llama3.1 / etc.）が動くか |
...
ベンチマークは Issue #17 で採用モデル確定後、`scripts/serve_local_llm.sh` を
vLLM / sglang で起動して 1発言推論を 100 回回し、p50 / p95 を計測する。
